...
方法一，從 Client (Mac/PC) 下指令，每秒監控一次 Server (DGX Spark) 的 GPU 溫度 (讀GPU Temp欄位數值) 、GPU使用率 (讀GPU-Util欄位數值)
```
watch -n 1 nvidia-smi
```

方法二，從 Client (Mac/PC) 下指令，每秒監控一次 Server (DGX Spark) 系統記憶體的 總量(讀total欄位數值)、當前使用量(讀used欄位數值)
